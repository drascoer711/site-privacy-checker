...
function summarizeSecurity(headers) {
  const present = Object.entries(headers).filter(([, value]) => Boolean(value)).map(([name]) => name);
  ...
  return { total: SECURITY_HEADERS.length, present: present.length, missing, score };
}

function cleanLabel(value) {
  return String(value || 'Anonymous visitor').replace(/[\\`*_~|<>]/g, '').trim().slice(0, 80) || 'Anonymous visitor';
}
...
function parseCookieList(cookieHeaders) {
  return cookieHeaders.map((cookie) => {
    ...
    return {
      ...
      risky: /(session|track|analytics|_ga|_gid|fbp|tt|utm)/i.test(name || '') || /secure|httponly|samesite/i.test(attributes.join(' ')) === false
    };
  });
}
...
async function notifyDiscord({ target, requester, result, req }) {
  ...
  try {
    ...
    // swallow webhook failure
  }
}

async function performBrowserScan(targetUrl) {
  let playwright;
  try {
    playwright = await import('playwright');
  } catch {
    return { available: false, reason: 'Playwright not available in this environment.' };
  }

  const browser = await playwright.chromium.launch({ headless: true });
  const page = await browser.newPage({ userAgent: 'Tracecheck/1.0 (privacy audit)' });
  const requests = [];
  const cookies = [];
  const redirects = [];
  ...
  try {
    ...
    const allCookies = pageCookies.map((c) => ({ name: c.name, value: c.value, domain: c.domain, path: c.path, secure: c.secure, httpOnly: c.httpOnly, sameSite: c.sameSite }));
    ...
    return {
      ...
      requests: [...new Set(requests)].slice(0, 50),
      redirects: redirects.slice(0, 20),
      cookies: allCookies,
      storage,
      note: 'Browser test ran in a headless Chromium session.'
    };
  } catch (error) {
    return {
      available: false,
      reason: error.message || 'Browser scan failed.',
      requests: [],
      cookies: [],
      redirects: [],
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let response;
    try {
      response = await fetch(target, { signal: controller.signal, redirect: 'manual', headers: { 'User-Agent': 'Tracecheck/1.0 (privacy audit)' } });
    } finally {
      clearTimeout(timer);
    }
    ...
    for (const tracker of TRACKERS) {
      if (tracker.pattern.test(html)) {
        findings.push({ category: 'Tracker', name: tracker.name, severity: tracker.severity, detail: 'A matching script or API pattern was found in the page.' });
      }
    }

    for (const signal of FINGERPRINTING) {
      if (signal.pattern.test(html)) {
        findings.push({ category: 'Fingerprinting', name: signal.name, severity: 'high', detail: 'A browser/device identification API pattern was found in page source.' });
      }
    }

    const cookieHeaders = response.headers.getSetCookie?.() || (response.headers.get('set-cookie') ? [response.headers.get('set-cookie')] : []);
    const cookies = parseCookieList(cookieHeaders);
    for (const cookie of cookies) {
      if (cookie.risky) {
        findings.push({ category: 'Cookie', name: cookie.name, severity: /analytics|track|_ga|_gid|fbp|tt/i.test(cookie.name) ? 'medium' : 'low', detail: `Cookie attributes: ${cookie.attributes.join(', ') || 'no explicit attributes'}` });
      }
    }

    const headers = {};
    for (const name of SECURITY_HEADERS) headers[name] = response.headers.get(name) || null;

    const externalDomains = [...new Set(
      [...html.matchAll(/(?:src|href|action)=["']([^"']+)["']/gi)]
        .map((m) => {
          try { return new URL(m[1], target).hostname; } catch { return null; }
        })
    ...
    const technologies = TECHNOLOGIES.filter(([, pattern]) => pattern.test(html)).map(([name]) => name);
    const security = summarizeSecurity(headers);
    const risk = severityScore(findings);
    const browserScan = await performBrowserScan(target.href);
    ...
    const result = {
      ...
      browser: browserScan,
      note: 'HTML and response headers were inspected; JavaScript was not executed unless browser automation is available.'
    };

    const saved = await saveReport(result);
    await notifyDiscord({ target, requester, result: saved, req });

    return res.status(200).json({ ...saved, reportId: saved.id });
  } catch (error) {
    return res.status(400).json({ error: error.name === 'AbortError' ? 'The scan timed out.' : error.message || 'The scan failed.' });
  }
}
