...
const TECHNOLOGIES = [
  ["WordPress", /wp-content|wp-includes/i], ["Shopify", /cdn\.shopify\.com|shopifycdn/i],
  ["Webflow", /webflow\.com|data-wf-page/i], ["React", /react(?:\.production)?\.min\.js|__next_data__/i],
  ["Next.js", /_next\/static|__next_f/i], ["Vue", /vue(?:\.runtime)?\.min\.js/i],
...
function parseTarget(value) {
  ...
  if (!["http:", "https:"].includes(url.protocol) || url.username || url.password || isPrivateHost(url.hostname)) throw new Error("Only public HTTP(S) URLs without credentials can be scanned.");
  url.hash = "";
  return url;
}
...
async function notifyDiscord({ target, requester, result, req }) {
  ...
  try {
    await fetch(webhook, {
      ...
      body: JSON.stringify({ username: "Tracecheck", embeds: [{ title: "New privacy scan", description: `A visitor requested a scan of ${target.href}`, color: result.risk.level === "high" ? 0xff7890 : result.risk.level === "medium" ? 0xffd166 : 0x74f5ca, fields, timestamp: new Date().toISOString(), footer: { text: "Tracecheck · IP addresses are not included" } }] })
    });
  } catch { /* Reporting must never make a scan fail. */ }
}
export default async function handler(req, res) {
  ...
  try {
    ...
    let response;
    try { response = await fetch(target, { signal: controller.signal, redirect: "manual", headers: { "User-Agent": "Tracecheck/1.0 (privacy audit)" } }); }
    finally { clearTimeout(timer); }
    const contentType = response.headers.get("content-type") || "";
    if (!contentType.includes("text/html")) return res.status(422).json({ error: "The URL did not return an HTML page." });
    const reader = response.body?.getReader(); let bytes = 0; let html = "";
    if (reader) {
      const decoder = new TextDecoder();
      while (true) { const { value, done } = await reader.read(); if (done) break; bytes += value.byteLength; if (bytes > MAX_BYTES) break; html += decoder.decode(value, { stream: true }); }
    } else html = await response.text();
    const lower = html.toLowerCase(); const findings = [];
    ...
    const headers = {}; for (const name of ["content-security-policy", "referrer-policy", "permissions-policy", "strict-transport-security", "x-content-type-options", "x-frame-options"]) headers[name] = response.headers.get(name) || null;
    ...
    const securityHeaders = Object.entries(headers).filter(([, value]) => value).length;
    const result = { url: target.href, status: response.status, truncated: bytes > MAX_BYTES, risk: severityScore(findings), findings, externalDomains, technologies, headers, security: { present: securityHeaders, total: Object.keys(headers).length }, fetchedAt: new Date().toISOString(), note: "HTML and response headers were inspected; JavaScript was not executed." };
    await notifyDiscord({ target, requester: req.body?.requester, result, req });
